...
namespace DG244Cutting.A_Domain.Interfaces.Repositories.DIGIT_TRY
{
    ...
    /// les séries de production, à destination de l'écran de consultation d'une série et de
    /// l'écran de validation des barres.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// l'optimisation. La première relève de la consultation : « quelles barres ont été retenues
    /// pour telle série de production, avec les seules caractéristiques que l'écran de
    /// consultation affiche ». Les deux autres relèvent de la validation des barres par
    /// l'opérateur : « quelle est la barre qui lui est présentée », afin qu'il en vérifie la
    /// matière avant de l'accepter, et « quelles barres de la série sont mises en attente pour
    /// rupture de stock », afin qu'il voie ce qui bloque la série et puisse libérer une barre dont
    /// la matière est revenue. Ces barres proviennent soit du stock de chutes issues de séries
    /// antérieures, soit du stock de barres neuves ; chacune porte, outre ses caractéristiques
    /// d'article et de profilé, cinq indicateurs d'état qui jalonnent son parcours - barre neuve ou
    /// chute, barre validée par l'opérateur, barre effectivement utilisée, barre en rupture de
    /// stock, barre refusée, ce dernier cas s'accompagnant d'un motif conservé sur
    /// l'enregistrement. La vue source expose quatre-vingt-deux colonnes.
    /// </para>
    /// <para>
    /// Les trois lectures rapatrient les mêmes dix-huit champs, sous le même type de projection :
    /// les seize champs d'affichage de l'onglet de consultation des barres de la Page11, plus deux
    /// champs de service non affichés servant à l'identification des lignes, à la vérification de
    /// cohérence du lot reçu et à l'ordonnancement. L'écran de validation des barres de la Page20
    /// y puise, pour la barre présentée, la référence, la désignation, la couleur, la longueur,
    /// l'origine neuve ou chute et le nombre de découpes que le plan y place.
    /// </para>
    /// <para>
    /// Portée du résultat - lecture Page11 : à la différence des autres lectures du projet, cette
    /// lecture ne filtre pas les enregistrements marqués comme logiquement supprimés. Le refus
    /// d'une barre par l'opérateur marque précisément l'enregistrement de cette façon, et l'écran
    /// doit afficher ces barres refusées avec leur motif : elles font partie intégrante du
    /// résultat attendu. Une série dont l'optimisation n'a pas encore été lancée ne porte aucune
    /// barre ; une liste vide est alors un résultat nominal et non une erreur.
    /// </para>
    /// <para>
    /// Portée du résultat - lectures Page20 : la lecture de la barre présentée ne filtre aucun
    /// indicateur d'état ; la barre désignée est rendue quel que soit son état, qu'elle soit
    /// validée, utilisée, en rupture de stock ou refusée. L'absence de ligne pour l'identifiant
    /// fourni produit une valeur absente, qui n'est pas une erreur au niveau du repository : la
    /// barre ayant été désignée en amont, ce retour traduit une incohérence dont le traitement
    /// appartient à l'appelant. La lecture des barres en rupture de la série retient les barres
    ...
    /// consommation directe du contrat générique imposerait de matérialiser les quatre-vingt-deux
    /// colonnes puis d'en écarter soixante-quatre en mémoire, ce qui relèverait du Cas 2
    /// (transformation LINQ-to-Objects portée par un Query Handler) et ferait perdre la réduction
    /// côté base qui est la finalité même du composant. La lecture des barres en rupture requiert
    /// en outre un tri sur deux colonnes, que le socle n'expose pas. Conformément au sous-bloc
    /// « Critère de création d'un Repository spécialisé CR_[Entité] » de §4.14.6 du 0230, la
    /// projection SQL traduite côté base est nominativement rangée au Cas 3, seul cas justifiant
    /// la création d'un repository spécialisé.
    /// </para>
    ...
    ///     contrat générique : la lecture de consultation de la Page11 et les deux lectures
    ///     d'affichage de la Page20 - barre présentée et barres en rupture de la série -, toutes
    ///     réduites de quatre-vingt-deux à dix-huit colonnes. La réduction est dans tous les cas
    ...
    ///     est sans objet pour la lecture de la barre présentée, qui rend au plus une barre.
    ///   </description></item>
    ///   <item><description>
    ...
    public interface IR_VwProductionBarFull : IR_Generic<vw_ProductionBar_Full>
    {
        ...
        /// <remarks>
        /// <para>
        /// La lecture sert l'onglet de détail de la barre présentée : l'opérateur y vérifie la
        /// matière - référence, désignation, couleur, longueur, origine neuve ou chute, nombre de
        /// découpes placées - avant d'accepter la barre. Le type rendu est
        ...
        /// <remarks>
        /// <para>
        /// La lecture sert l'onglet des barres en rupture de la série : l'opérateur y voit ce qui
