...
namespace DG244Cutting.A_Domain.Interfaces.Repositories.DIGIT_TRY
{
    ...
    /// </para>
    /// <para>
    /// Objectif : servir la question fermée « quelles barres ont été retenues pour telle série de
    /// production, avec les seules caractéristiques que l'écran de consultation affiche ». Ces
    /// barres proviennent soit du stock de chutes issues de séries antérieures, soit du stock de
    /// barres neuves ; chacune porte, outre ses caractéristiques d'article et de profilé, cinq
    ...
    /// quatre-vingt-deux colonnes ; le quatrième onglet de la Page11 en affiche seize. La lecture
    /// exposée par le présent contrat rapatrie dix-huit champs - les seize champs d'affichage,
    /// plus deux champs de service non affichés servant à l'identification des lignes, à la
    /// vérification de cohérence du lot reçu et à l'ordonnancement laissé à la charge de
    /// l'appelant.
    /// </para>
    /// <para>
    /// Portée du résultat : à la différence des autres lectures du projet, la lecture exposée ne
    /// filtre pas les enregistrements marqués comme logiquement supprimés. Le refus d'une barre
    /// par l'opérateur marque précisément l'enregistrement de cette façon, et l'écran doit
    /// afficher ces barres refusées avec leur motif : elles font partie intégrante du résultat
    /// attendu. Une série dont l'optimisation n'a pas encore été lancée ne porte aucune barre ;
    ...
    ///     générique, et dont la réduction de quatre-vingt-deux à dix-huit colonnes est traduite
    ...
    public interface IR_VwProductionBarFull : IR_Generic<vw_ProductionBar_Full>
    {
        ...
        /// </exception>
        Task<List<DTO_VwProductionBarFull_P11>> GetByProductionSeriesIdForP11AsNoTrackingAsync(
            string caller,
