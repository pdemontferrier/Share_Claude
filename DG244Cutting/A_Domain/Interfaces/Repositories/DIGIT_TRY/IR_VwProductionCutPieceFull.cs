...
namespace DG244Cutting.A_Domain.Interfaces.Repositories.DIGIT_TRY
{
    ...
    /// les séries de production, à destination de l'écran de consultation d'une série, du
    /// moteur d'optimisation des barres et de l'onglet de contrôle du plan de coupe d'une barre.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Objectif : servir quatre questions fermées portant sur les découpes de production, trois à
    /// l'échelle d'une série et une à l'échelle d'une barre. La première relève de la
    /// consultation : « quelles découpes composent telle série, avec les seules caractéristiques
    /// que l'écran affiche ». Les deux suivantes relèvent de l'optimisation des barres : « quelle
    /// est la prochaine découpe à traiter dans la série » et « quelles découpes restent à placer
    /// pour tel article interne de la série ». La dernière relève du contrôle opérateur : « quel
    /// plan de coupe est posé sur telle barre, dans quel ordre de réalisation ». La découpe est
    ...
    /// recherche de la prochaine référence et présentent le profilé à l'opérateur. La lecture
    /// destinée à l'onglet de contrôle du plan de coupe de la Page20 rapatrie les mêmes vingt
    /// champs que la lecture Page11 : les seize champs d'affichage et les quatre champs de
    /// service.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Portée du résultat - lectures d'optimisation de la Page20 : ces deux lectures ne retiennent
    /// que les découpes disponibles pour l'optimisation, c'est-à-dire ni coupées, ni engagées dans une
    ...
    /// absente pour la recherche de la prochaine découpe, une liste vide pour le vivier. Ce
    /// critère de disponibilité reste propre aux deux lectures d'optimisation : il écarte par
    /// construction les découpes engagées dans une optimisation, c'est-à-dire celles-là mêmes que
    /// la lecture du plan de coupe restitue.
    /// </para>
    /// <para>
    /// Portée du résultat - lecture du plan de coupe : cette lecture retient les découpes
    /// rattachées à la barre de production demandée, la barre suffisant à désigner le plan sans
    /// référence à la série. Les découpes supprimées logiquement sont écartées : une découpe
    /// supprimée encore rattachée à une barre est une incohérence qui ne doit pas être présentée
    /// à l'opérateur. Les découpes dont la barre est en rupture de stock sont restituées, cet état
    /// étant nominal et réversible pour une barre présentée ; les découpes refusées le sont
    /// également. Le résultat est ordonné côté base de données dans l'ordre de réalisation : les
    /// positions de coupe renseignées d'abord, par position croissante puis par clé technique
    /// croissante, les découpes rattachées sans position ensuite, en fin de liste, où l'anomalie
    /// reste visible sans perturber l'ordre nominal. Une barre sans découpe rattachée produit une
    /// liste vide, résultat nominal et non erreur. La lecture ne vérifie pas que la barre porte
    /// effectivement un plan de coupe : cette condition relève de l'appelant.
    /// </para>
    /// <para>
    /// Justification du Patron 2 : cette interface n'existe que parce que les quatre lectures de
    ...
    /// et pour la lecture du plan de coupe, et deux cent vingt pour les lectures d'optimisation
    /// de la Page20, ce qui relèverait du Cas 2 (transformation LINQ-to-Objects portée par un
    /// Query Handler) et ferait perdre la réduction côté base qui est la finalité même du
    /// composant. La recherche de la prochaine découpe requiert en outre un tri sur deux colonnes,
    /// et la lecture du plan de coupe un tri sur trois clés, ce que le socle n'expose pas.
    /// Conformément au sous-bloc « Critère de création d'un Repository spécialisé CR_[Entité] »
    /// de §4.14.6 du 0230, la projection SQL traduite côté base est nominativement rangée au
    /// Cas 3, seul cas justifiant la création d'un repository spécialisé.
    /// </para>
    ...
    ///   </description></item>
    ///   <item><description>
    ///     Déclarer les quatre lectures projetées qui mobilisent la projection SQL absente du
    ///     contrat générique : la lecture de consultation de la Page11, réduite à vingt colonnes,
    ///     les deux lectures du moteur d'optimisation de la Page20 - recherche de la prochaine
    ///     découpe à traiter et lecture du vivier d'un article interne -, réduites à treize
    ///     colonnes, et la lecture du plan de coupe d'une barre pour l'onglet de contrôle de la
    ///     Page20, réduite à vingt colonnes. La réduction est dans tous les cas traduite en
    ...
    ///     extraction. Les trois lectures de la Page20 - les deux lectures d'optimisation et la
    ///     lecture du plan de coupe - ordonnent en revanche leur résultat côté base de données,
    ///     l'ordre étant constitutif de leur contrat.
    ///   </description></item>
    ///   <item><description>
    ...
    ///     d'optimisation de la Page20, dont le filtrage se limite à la série, à la disponibilité
    ///     pour l'optimisation et, pour le vivier, à l'article interne, ni pour la lecture du plan
    ///     de coupe, dont le filtrage se limite à la barre et à l'exclusion des découpes
    ...
    public interface IR_VwProductionCutPieceFull : IR_Generic<vw_ProductionCutPiece_Full>
    {
        ...
        /// <remarks>
        /// <para>
        /// Le plan de coupe est la suite ordonnée des découpes que l'optimisation a posées sur une
        /// barre, chacune à sa position. Il est présenté à l'opérateur avant l'acceptation de la
        /// barre, afin qu'il en vérifie les références, les désignations, les couleurs, la
        /// géométrie de coupe de chaque pièce et l'ordre de réalisation. Le rattachement à la
        /// barre suffit à désigner le plan : la série n'est pas demandée.
        /// </para>
        /// <para>
        ...
        /// conservés. Une découpe refusée (<c>PCPIsCutRefused</c>) est également restituée. Aucun
        /// recours n'est fait à <c>IgnoreQueryFilters</c> : aucun filtre global n'est configuré
        /// sur le contexte de données. La lecture ne vérifie pas que la barre porte effectivement
        /// un plan de coupe : cette condition relève de l'appelant.
        /// </para>
        /// <para>
        /// L'ordre de restitution est celui de la réalisation et fait partie du contrat : les
        /// découpes dont la position de coupe <c>PCPCutPositionInBar</c> est renseignée viennent
        /// d'abord, par position croissante puis par clé technique <c>PCPId</c> croissante ; les
        /// découpes rattachées sans position de coupe viennent ensuite, en fin de liste, selon le
        /// même départage. Une découpe rattachée sans position est une anomalie tolérée : elle
        /// reste visible sans perturber l'ordre des lignes nominales.
        /// </para>
        /// <para>
        /// Chaque ligne recopie terme à terme, types et nullabilité inclus, les vingt champs de la
        /// lecture Page11 : seize champs d'affichage et quatre champs de service. La projection
        /// emploie <see cref="DTO_VwProductionCutPieceFull_P11"/>, type conçu pour la consultation
        /// de la Page11 et dont la composition couvre les besoins du contrôle du plan de coupe.
        /// Cet emprunt est un écart assumé et provisoire : un type d'affichage propre à l'onglet
        /// de contrôle est appelé à le remplacer, la substitution ne touchant que la clause de
        /// projection et la signature. Le nombre de lignes n'est pas borné et aucune pagination
        /// n'est appliquée.
        /// </para>
        /// <para>
        /// Justification du Cas 3 (§4.14.6 du 0230) : la projection <c>Select</c> vers un type
        ...
        /// </para>
        /// <para>
        /// La lecture est pure : aucun suivi, aucune écriture, aucune journalisation. Elle
        /// n'emprunte aucune des méthodes héritées inopérantes sur ce type sans clé.
        /// </para>
        /// </remarks>
        /// <param name="caller">CallChain construite par le composant appelant.</param>
        ...
        /// réalisation, jamais <see langword="null"/>. Liste vide si aucune découpe non supprimée
        /// n'est rattachée à la barre : ce résultat est nominal et ne constitue pas une erreur,
        /// son interprétation revenant à l'appelant.
        /// </returns>
        /// <exception cref="Ex_Business">
        ...
        /// Levée si l'annulation est signalée via <paramref name="ct"/> avant ou pendant l'exécution.
        /// </exception>
        Task<List<DTO_VwProductionCutPieceFull_P11>> GetByProductionBarIdForP20AsNoTrackingAsync(
            string caller,
            int idProductionBar,
            CancellationToken ct = default);
    }
