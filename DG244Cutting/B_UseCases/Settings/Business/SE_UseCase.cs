...
{
    /// <summary>
    /// État des sélections métier courantes partagé par l'application.
    /// </summary>
    /// <remarks>
    ...
    public class SE_UseCase : ISE_UseCase
    {
        ...
        /// <summary>
        /// État complet de la série sélectionnée, ou <see langword="null"/> lorsqu'aucune série n'est sélectionnée.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Identifiant de la série sélectionnée en tête de la cascade de sélection métier.
        /// </summary>
        /// <remarks>
        /// La valeur <c>0</c> est la sentinelle conventionnelle « aucune sélection ». Cet identifiant est dérivé de
        /// <see cref="SelectedSeries"/> et lui reste cohérent. Écriture exclusive via <see cref="SelectSeries"/> (positionnement)
        /// et <see cref="Reset"/> (remise à <c>0</c>).
        /// </remarks>
        public int IdSeriesSelected => _idSeriesSelected;

        /// <summary>
        /// Identifiant de la barre sélectionnée au sein de la série courante.
        /// </summary>
        /// <remarks>
        /// La valeur <c>0</c> est la sentinelle conventionnelle « aucune sélection ».
        /// Écriture exclusive via <see cref="SelectSeries"/> et <see cref="Reset"/> (réinitialisation),
        /// <see cref="SelectBar"/> (positionnement) et <see cref="ClearBar"/> (désélection).
        /// </remarks>
        public int IdBarSelected => _idBarSelected;

        /// <summary>
        /// Identifiant de la découpe sélectionnée au sein de la barre courante.
        /// </summary>
        /// <remarks>
        ...
        /// en outre réinitialisée à <c>0</c> par <see cref="SelectSeries"/>, <see cref="SelectBar"/>,
        /// <see cref="ClearBar"/> et <see cref="Reset"/>.
        /// </remarks>
        ...
        /// (<see cref="SelectSeries"/>, <see cref="SelectBar"/>, <see cref="ClearBar"/>, <see cref="Reset"/>).</para>
        ...
        /// <see cref="SetField{T}"/> sur chaque champ support concerné. Le retrait de la barre relève de
        /// <see cref="ClearBar"/> et la réinitialisation globale de <see cref="Reset"/> ; aucune validation de
        /// positivité ni garde de cohérence amont n'est réalisée, la cohérence de la cascade reposant sur l'ordre
        /// d'appel des consommateurs.</para>
        /// </remarks>
        /// <param name="idBar">Identifiant de la barre à sélectionner.</param>
        ...
        /// <summary>
        /// Retire la barre sélectionnée et invalide la découpe héritée, sans toucher à la série courante.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Appelée par un consommateur amont lorsque la barre courante est écartée tout en
        /// conservant la série sélectionnée.</para>
        /// <para>Objectif : Ramener atomiquement <see cref="IdBarSelected"/> puis <see cref="IdCuttingSelected"/>
        /// à la sentinelle <c>0</c>, <see cref="SelectedSeries"/> et <see cref="IdSeriesSelected"/> restant inchangés,
        /// en invoquant directement <see cref="SetField{T}"/> avec <c>nameof</c> sur chaque champ support, ce qui
        /// filtre les valeurs identiques et notifie uniquement les propriétés réellement modifiées. L'opération ne
        /// comporte aucune précondition ; appelée alors qu'aucune barre n'est sélectionnée, elle est sans effet et
        /// ne constitue pas une erreur. Elle se distingue de <see cref="SelectSeries"/>, qui repositionne la série,
        /// et de <see cref="Reset"/>, qui réinitialise la cascade entière.</para>
        /// </remarks>
        ...
        /// atomiques multi-propriétés <see cref="SelectSeries"/>, <see cref="SelectBar"/>, <see cref="ClearBar"/>
        /// et <see cref="Reset"/>
