...
{
    /// <summary>
    /// Service métier responsable du rattachement provisoire, à une barre de production, des
    ...
    /// <see cref="IS_ProductionCutPiece_AssignToBar"/>, par le UseCase orchestrateur de
    /// l'optimisation de barre et par le UseCase orchestrateur de la validation de barre lorsqu'une
    /// ré-optimisation est requise. Il consomme directement <see cref="IQ_Generic{T}"/> pour la
    /// lecture préalable des découpes et <see cref="IC_Generic{T}"/> pour leur mise à jour, sur
    /// l'entité <see cref="ProductionCutPiece"/>.
    /// </para>
    /// <para>
    /// Objectif : inscrire sur les découpes elles-mêmes la décision de l'application de les couper
    /// sur une barre donnée. Chaque découpe quitte le vivier des pièces à optimiser, se rattache à
    /// la barre et reçoit son rang dans le plan de coupe, rang qui fixe l'ordre dans lequel
    /// l'opérateur réalisera les découpes. Le service est le point unique de traduction de l'ordre
    /// calculé en positions de coupe persistantes ; il marque les découpes puis en délègue la mise à
    /// jour au Command Handler générique, sans exposer la logique de persistance ni assumer de
    ...
    /// </para>
    /// <para>
    /// Le placement est provisoire : les découpes sont marquées comme provisoirement placées sans
    /// être scellées, le scellement du plan de coupe relevant de la validation de la barre. Une
    /// découpe précédemment refusée sur une autre barre, revenue au vivier avec son marqueur de
    /// refus, retrouve par ce rattachement un statut neutre. Le traitement est intégral : toutes les
    /// découpes sont marquées ou aucune ; tout échec interrompt le traitement avant délégation, et la
    /// transaction de l'appelant est annulée.
    /// </para>
    ...
    /// <item><description>Positionner sur chaque découpe le marqueur de placement provisoire, la levée du marqueur de refus, la barre de rattachement et la position de coupe.</description></item>
    ...
    /// <item><description>Ne crée pas la barre et ne vérifie pas son existence : l'intégrité référentielle entre la découpe et la barre est signalée par la persistance déclenchée par l'appelant.</description></item>
    ...
    /// <item><description>Ne journalise ni ne notifie, et n'inscrit aucune action de cycle de vie : la traçabilité métier commence à la validation de la barre.</description></item>
    /// </list>
    /// </remarks>
    /// <seealso cref="IS_ProductionCutPiece_AssignToBar"/>
    /// <seealso cref="IQ_Generic{T}"/>
    /// <seealso cref="IC_Generic{T}"/>
    public class SR_ProductionCutPiece_AssignToBar : IS_ProductionCutPiece_AssignToBar
    {
        ...
        /// <summary>
        /// Initialise une nouvelle instance de <see cref="SR_ProductionCutPiece_AssignToBar"/> avec ses dépendances.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        public SR_ProductionCutPiece_AssignToBar(
        ...
        /// dans la liste reçue, puis confie leur mise à jour au Command Handler générique.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        /// barre. Les découpes sont lues en une requête unique avec suivi des changements ; le
        /// Command Handler générique positionne ensuite la date de mise à jour et inscrit un
        /// événement par découpe. L'enregistrement effectif n'intervient qu'à la validation de la
        ...
        /// <item><description>Vérifier l'état de chaque découpe et rejeter en un échec unique l'ensemble des découpes déjà rattachées à une barre, déjà réalisées ou supprimées logiquement.</description></item>
        /// <item><description>Parcourir la liste reçue dans son ordre et, pour chaque identifiant de rang <c>i</c> compté à partir de 0, retrouver la découpe par son identifiant puis positionner le marqueur de placement provisoire à <see langword="true"/>, le marqueur de refus à <see langword="false"/>, la barre de rattachement et la position de coupe <c>i + 1</c>.</description></item>
        /// <item><description>Transmettre au Command Handler générique les instances chargées et marquées.</description></item>
        /// </list>
        /// <para>Non-responsabilités :</para>
        /// <list type="bullet">
        /// <item><description>Ne modifie aucun autre champ : l'optimisation scellée, l'approvisionnement de la barre, la réalisation, la suppression logique, la date de création et la date de mise à jour restent en dehors du marquage.</description></item>
        ...
        /// compté à partir de 1, devient la position de coupe. Ne doit pas être
        /// <see langword="null"/> ni vide ; chaque identifiant doit être strictement positif et
        /// n'apparaître qu'une seule fois.
        /// </param>
        /// <param name="idProductionBar">Identifiant de la barre de production de rattachement, attribué par la persistance. Doit être strictement positif.</param>
        ...
        /// des découpes sont déjà rattachées à une barre, déjà réalisées ou supprimées logiquement
        /// (chaque découpe et chaque condition violée citées). Remonte également sans interception
        /// toute <see cref="Ex_Business"/> levée par le Query Handler ou le Command Handler.
        /// </exception>
            ...
            try
            {
                ...
                if (pieceIds is null)
                    throw new Ex_Business(
                        ...
                        "La liste des identifiants de découpes (pieceIds) est obligatoire pour le rattachement à une barre de production.");
                ...
                if (pieces.Count != pieceIds.Count)
                {
                    ...
                    throw new Ex_Business(
                        callChain,
                        Ex_Business.ErrorCodes.BU_ER_03,
                        $"Des découpes désignées sont introuvables, signe d'une incohérence entre le calcul d'optimisation et l'état persistant ; identifiants introuvables : {string.Join(", ", missingIds)}.");
                }
                ...
                // C2 - État compatible avec le rattachement, contrôlé sur chaque découpe ; les
                // découpes fautives sont citées dans l'ordre de la liste reçue, en un échec unique.
                ...
                if (stateViolations.Count > 0)
                    throw new Ex_Business(
                        ...
                        $"L'état des découpes suivantes ne permet pas leur rattachement à la barre de production {idProductionBar} : {string.Join(" ; ", stateViolations)}.");
                ...
                for (int i = 0; i < pieceIds.Count; i++)
                {
                    ...
                    piece.IsOptimizedTemp = true;
                    piece.IsCutRefused = false;
                    piece.IdProductionBar = idProductionBar;
                    piece.CutPositionInBar = i + 1;
                }

                // D - Délégation des mêmes instances que celles chargées ; UpdatedAt et
        ...
        /// <summary>
        /// Décrit les conditions d'état qui interdisent le rattachement d'une découpe à une barre de
        ...
        /// rattachable si elle n'est pas déjà rattachée à une barre, n'a pas été réalisée et n'est
        /// pas supprimée logiquement ; toutes les conditions violées sont citées ensemble afin que
