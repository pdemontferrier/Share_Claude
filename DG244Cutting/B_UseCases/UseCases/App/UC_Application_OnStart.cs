...
namespace DG244Cutting.B_UseCases.UseCases.App
{
    ...
    /// <para>Objectif : Orchestrer linéairement les dix étapes du Jalon 3 dans l'ordre
    ...
    ///   <see cref="IU_UserAppSession_Open"/> en sous-séquence à l'étape 7
    ...
    ///   <see cref="Ex_Business"/> code <see cref="Ex_Business.ErrorCodes.BU_ER_04"/> à l'étape 7
    ...
    ///   <c>"No_EC_01"</c>, homogène avec les refus applicatifs portés par les étapes 4, 5 et 10.</description></item>
    ...
    ///   sous-séquence à l'étape 9 (<see cref="ApplyUserPageRightAsync"/>) conformément à
    ...
    ///   <see cref="Ex_Business"/>, divergeant du patron homogène des étapes 4, 5, 7 et 10
    ...
    ///   <c>IS_Language</c> par <see cref="IU_Language_Apply"/> à l'étape 2
    ...
    ///   l'étape 9 (cf. 4ème dérogation ci-dessus), évitant par analogie stricte la
    ...
    ///   <item><description>Insertion d'une nouvelle Étape 1 orchestrée en tête de
    ///   séquence portant la tentative d'acquisition d'unicité d'instance sur la session
    ///   Windows courante via consommation du Service d'infrastructure
    ///   <see cref="IS_SingleInstanceGuard"/>, levée par le présent fil d'Extension
    ///   <c>UC_ApplicationOnStart_Extension</c>. La séquence orchestrée par
    ///   <see cref="ExecuteAsync"/> passe de neuf à dix étapes, avec décalage des neuf
    ///   étapes précédentes en positions 2 à 10 (renumérotation textuelle globale
    ///   scripturaire cohérente sur <c>&lt;summary&gt;</c>, <c>&lt;remarks&gt;</c> et
    ///   commentaires inline). La nouvelle Étape 1 suit le Patron A de propagation
    ///   directe par valeur du retour <see langword="false"/> du guard — silence pur en
    ///   cas de seconde instance détectée avec signalement à l'instance primaire
    ///   préexistante, sans notification utilisateur, sans levée d'
    ///   <see cref="Ex_Business"/> interne, sans passage par le catch terminal
    ///   <see cref="IU_LogAndNotify"/> ; <c>App.xaml.cs</c> déclenche alors le Jalon 4b
    ///   (§3.10.6 du 0230). Position du paramètre <c>singleInstanceGuard</c> dans la
    ///   signature du constructeur PAR REGROUPEMENT DE FAMILLE avec les autres Services
    ///   <c>IS_</c> existants (immédiatement après
    ///   <see cref="IS_DigitTryDb_TestConnection"/> et avant le premier Query Handler
    ///   <see cref="IQ_UserApp"/>), en arbitrage explicite du développeur en faveur du
    ///   regroupement par famille technologique — divergence assumée par rapport à la
    ///   convention par défaut d'« ajout en queue » couramment retenue pour les
    ///   Extensions de couples UseCase existants, tracée nominativement au titre de
    ///   §1.4.8 du 0230. La conformité de l'ajout au regard de l'item UC6 (garde
    ///   <c>ArgumentNullException</c> symétrique aux gardes existantes) et de la portée
    ///   DI (portée <c>Scoped</c> du UseCase inchangée, la nouvelle dépendance
    ///   <see cref="IS_SingleInstanceGuard"/> étant enregistrée <c>Singleton</c> par
    ///   ailleurs — cf. §7 du présent fil d'Extension) n'est pas requalifiée par cet
    ///   arbitrage de position, qui n'est pas normé par le 0230. La nouvelle
    ///   dépendance <see cref="IS_SingleInstanceGuard"/> est un Service d'infrastructure
    ///   (famille <c>IS_</c>) et ne constitue PAS une consommation UC → UC en
    ///   sous-séquence au sens de R-4.14.21 ; item UC22 demeure ✅ à trois consommations
    ///   inchangées ; item UC21 demeure ➖ pérenne (consommateur amont
    ///   <c>App.xaml.cs</c> hors famille UseCases).</description></item>
    /// </list>
    ...
    /// linéairement dix méthodes privées correspondant aux dix étapes du Jalon 3
    /// (cf. région <c>=== Méthodes privées ===</c>). Pour les chemins de refus applicatif des
    /// étapes 4, 5, 7 et 10, une <see cref="Ex_Business"/> est levée intentionnellement avec
    ...
    /// patron strictement conforme à §4.7.2 du 0230 et R-4.7.3 du 0231. À l'étape 7, le refus
    /// est porté par conversion explicite du <see langword="false"/> retourné par
    /// <see cref="IU_UserAppSession_Open"/> en <see cref="Ex_Business"/> ; aux étapes 4, 5 et
    /// 10, le refus est levé directement par les méthodes privées correspondantes en cas
    /// d'état applicatif incompatible (connectivité base, disponibilité applicative, conflit
    /// de session). Les étapes 2 (<see cref="ApplyCultureAsync"/>) et 9
    ...
    /// (étape 2) et de <see cref="IU_UserAppPageRight_Apply.ExecuteAsync"/> (étape 9) est
    ...
    public sealed class UC_Application_OnStart : IU_Application_OnStart
    {
        ...
        /// <para>Objectif : Câbler les treize dépendances nécessaires à l'orchestration des
        /// dix étapes du Jalon 3.</para>
        /// </remarks>
        ...
        /// <param name="singleInstanceGuard">Service d'infrastructure de garde d'unicité d'instance applicative sur la session Windows courante, consommé en tête de séquence orchestrée (nouvelle Étape 1) au titre du présent fil d'Extension.</param>
        ...
        public async Task<bool> ExecuteAsync(string caller, string cultureCode, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                // Étape 1 - Tentative d'acquisition d'unicité d'instance (§3.10, nouvelle
                // étape introduite par le présent fil d'Extension). Patron de propagation
                // directe par valeur du retour signalable (silence pur en cas de retour
                // false — seconde instance signalée à la primaire par le guard ;
                // App.xaml.cs déclenche alors le Jalon 4b, Shutdown WPF).
                if (!TryAcquireSingleInstance(callChain, ct))
                    return false;

                // Étape 2 - Initialisation langue + dictionnaire + drapeau (§3.10, R-3.10.5).
                ...
                // Étape 3 - Mise à jour du titre applicatif (clé dictionnaire "App_Ti_00").
                LoadApplicationTitle(callChain, ct);

                // Étape 4 - Test de connectivité base de données + notification d'état.
                await CheckDatabaseConnectivityAsync(callChain, ct);

                // Étape 5 - Vérification de la disponibilité applicative (verrou administrateur).
                await CheckAppAvailabilityAsync(callChain, ct);

                // Étape 6 - Alimentation du contexte poste (§3.10.6, R-3.10.8).
                await LoadDeviceContextAsync(callChain, ct);

                // Étape 7 - Identification du DeviceUser et ouverture de session.
                await IdentifyDeviceUserAndOpenSessionAsync(callChain, ct);

                // Étape 8 - Chargement conditionnel du nom complet utilisateur (si AppUserId > 0).
                await LoadUserFullNameAsync(callChain, ct);

                // Étape 9 - Application des droits de pages (initialisation systématique au
                ...
                // Étape 10 - Vérification de l'intégrité de session (absence de conflit sur autre poste).
        ...
        /// <summary>
        /// Étape 1 — Tente d'acquérir l'unicité d'instance de l'application sur la session
        /// Windows courante en consommant <see cref="IS_SingleInstanceGuard"/>.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Première étape du Jalon 3, introduite par le fil d'Extension
        /// <c>UC_ApplicationOnStart_Extension</c>. Déléguée au Service d'infrastructure
        /// <see cref="IS_SingleInstanceGuard"/> qui encapsule l'usage des primitives noyau
        /// Windows (Mutex nommé de session et EventWaitHandle nommé de signalement) sous
        /// une frontière contractuelle abstraite en A_Domain. La méthode est SYNCHRONE (le
        /// contrat <see cref="IS_SingleInstanceGuard.Execute"/> est synchrone, retour
        /// <see langword="bool"/> direct sans <see cref="System.Threading.Tasks.Task"/>) —
        /// choix cohérent avec la nature des primitives Win32 mobilisées par
        /// l'implémentation.</para>
        /// <para>Objectif : Discriminer, en tête de séquence orchestrée, entre instance
        /// primaire acquise (<see langword="true"/> — la séquence se poursuit à l'étape 2)
        /// et seconde instance détectée (<see langword="false"/> — la seconde instance a
        /// été signalée à l'instance primaire préexistante en interne par le guard, la
        /// séquence sort immédiatement de <see cref="ExecuteAsync"/> par <c>return false</c>
        /// et <c>App.xaml.cs</c> déclenche le Jalon 4b, Shutdown WPF silencieux).</para>
        /// <para>Absence de try/catch local. Les catch typés terminaux applicatifs de
        /// <see cref="ExecuteAsync"/> (<see cref="Ex_Infrastructure"/> et
        /// <see cref="System.OperationCanceledException"/> notamment) capturent nativement
        /// les exceptions remontées par <see cref="IS_SingleInstanceGuard.Execute"/> :
        /// l'<see cref="Ex_Infrastructure"/> résulte d'une requalification terminale via
        /// <c>IS_ExClassifier</c> en cas de défaillance des primitives noyau Windows, captée
        /// par le bloc <c>catch (Ex_Infrastructure)</c> de <see cref="ExecuteAsync"/> qui
        /// délègue à <see cref="IU_LogAndNotify"/> avec clé <c>"No_EC_02"</c> ;
        /// l'<see cref="System.OperationCanceledException"/> est propagée par
        /// <c>throw</c> conformément au patron R-4.6.13. Patron homogène aux autres
        /// méthodes privées d'étape du présent UseCase.</para>
        /// <para>Position par regroupement de famille. La dépendance
        /// <see cref="IS_SingleInstanceGuard"/> est injectée au constructeur en position
        /// PAR REGROUPEMENT DE FAMILLE avec les autres Services <c>IS_</c> existants
        /// (immédiatement après <see cref="IS_DigitTryDb_TestConnection"/>), arbitrage
        /// explicite du développeur en faveur du regroupement par famille technologique —
        /// cf. 6ème dérogation du <c>&lt;remarks&gt;</c> de classe.</para>
        /// </remarks>
        private bool TryAcquireSingleInstance(string callChain, CancellationToken ct)
        {
            return _singleInstanceGuard.Execute(callChain, En_SingleInstanceOperation.Acquire, ct);
        }

        /// <summary>
        /// Étape 2 — Applique la langue applicative correspondant au code culture fourni en
        /// consommant <see cref="IU_Language_Apply"/> en sous-séquence.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Deuxième étape du Jalon 3, déléguée à
        ...
        /// dictionnaire consommé en aval (notamment à l'étape 3).</para>
        /// </remarks>
        ...
        /// <summary>
        /// Étape 6 — Alimente le contexte poste (<see cref="ISE_User"/>) de l'utilisateur courant.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Sixième étape du Jalon 3, déléguée à
        ...
        /// poste pour vérifier l'intégrité de session (étape 10) notamment.</para>
        /// </remarks>
        ...
        /// <summary>
        /// Étape 3 — Met à jour le titre applicatif via résolution dictionnaire de la clé
        /// <c>"App_Ti_00"</c>.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Troisième étape du Jalon 3. <see cref="IS_Dictionary.GetText"/>
        ...
        /// <summary>
        /// Étape 7 — Identifie l'utilisateur applicatif associé au login Windows du poste courant
        /// et orchestre l'ouverture de la session applicative en consommant
        /// <see cref="IU_UserAppSession_Open"/> en sous-séquence.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Septième étape du Jalon 3, exécutée après alimentation du
        /// contexte poste à l'étape 6 (<see cref="LoadDeviceContextAsync"/>). Lit le login Windows
        ...
        /// été transmis en ligne de commande au Jalon 2) et les étapes 8 et 10 sont sautées en
        ...
        /// refus ultérieur sur les étapes 9 ou 10 (l'étape 9 procède inconditionnellement à
        /// l'initialisation par défaut et ne refuse que sur référentiel <c>UserAppPage</c> vide ;
        /// l'étape 10 est sautée par sa garde <c>AppUserId &gt; 0</c>).</para>
        ...
        /// étapes 4 (<see cref="CheckDatabaseConnectivityAsync"/>), 5
        ...
        /// conformément à la doctrine §4.7 du 0230 et au patron des étapes 4, 5 et 10 en place.
        ...
        /// <summary>
        /// Étape 8 — Charge conditionnellement le nom complet de l'utilisateur applicatif courant
        /// si <c>ISE_User.AppUserId &gt; 0</c>.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Huitième étape du Jalon 3. L'<c>AppUserId</c> a été
        /// éventuellement initialisé au Jalon 2 par <c>App.xaml.cs.ProcessStartupArguments</c>
        /// (argument <c>"iduser=N"</c> en ligne de commande), ou à l'étape 7 par
        ...
        /// identifiant utilisateur n'a été transmis et que l'étape 7 n'a pas identifié
        ...
        /// l'étape 10 (<see cref="CheckSessionIntegrityAsync"/>) est ensuite sautée par sa propre
        ...
        /// <summary>
        /// Étape 9 — Applique les droits de pages dans le contexte utilisateur partagé en
        ...
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Neuvième étape du Jalon 3, exécutée après le chargement
        /// conditionnel du nom complet utilisateur à l'étape 8
        /// (<see cref="LoadUserFullNameAsync"/>) et avant la vérification de l'intégrité de
        /// session à l'étape 10 (<see cref="CheckSessionIntegrityAsync"/>). Délègue
        ...
        /// regard du patron des étapes 4, 5, 7 et 10 : Contrairement aux étapes 4
        /// (<see cref="CheckDatabaseConnectivityAsync"/>), 5
        /// (<see cref="CheckAppAvailabilityAsync"/>), 7
        /// (<see cref="IdentifyDeviceUserAndOpenSessionAsync"/>) et 10
        ...
        /// <summary>
        /// Étape 4 — Teste la connectivité à la base de données et notifie l'état correspondant
        ...
        /// <summary>
        /// Étape 5 — Vérifie la disponibilité applicative (verrou administrateur).
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Étape 10 — Vérifie l'absence de conflit de session active sur un autre poste pour le
        /// couple (utilisateur, application) courant.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Dixième étape du Jalon 3. Consomme
