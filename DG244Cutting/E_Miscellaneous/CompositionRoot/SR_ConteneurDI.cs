...
namespace DG244Cutting.E_Miscellaneous.CompositionRoot
{
    ...
    public static class SR_ConteneurDI
    {
        ...
        private static void RDI_Handlers(IServiceCollection services)
        {
            ...
            services.AddScoped<IQ_VwProductionCutPieceFull, QH_VwProductionCutPieceFull>();
        ...
        private static void RDI_Services(IServiceCollection services)
        {
            ...
            services.AddScoped<IS_CuttingScrapStock_ReleaseBySeries, SR_CuttingScrapStock_ReleaseBySeries>();
            services.AddScoped<IS_CuttingScrapStock_Reserve, SR_CuttingScrapStock_Reserve>();
            services.AddScoped<IS_CuttingScrapStock_Withdraw, SR_CuttingScrapStock_Withdraw>();
            services.AddScoped<IS_LifecycleAction_Add, SR_LifecycleAction_Add>();
            services.AddScoped<IS_ProductionBar_Create, SR_ProductionBar_Create>();
            services.AddScoped<IS_ProductionBar_Reject, SR_ProductionBar_Reject>();
            services.AddScoped<IS_ProductionBar_SetOutOfStock, SR_ProductionBar_SetOutOfStock>();
            services.AddScoped<IS_ProductionBar_UpdateOptimization, SR_ProductionBar_UpdateOptimization>();
            services.AddScoped<IS_ProductionBar_Validate, SR_ProductionBar_Validate>();
            services.AddScoped<IS_ProductionCutPiece_AssignAndSealToBar, SR_ProductionCutPiece_AssignAndSealToBar>();
            services.AddScoped<IS_ProductionCutPiece_AssignToBar, SR_ProductionCutPiece_AssignToBar>();
            services.AddScoped<IS_ProductionCutPiece_DetachAndRefuse, SR_ProductionCutPiece_DetachAndRefuse>();
            services.AddScoped<IS_ProductionCutPiece_DetachFromBar, SR_ProductionCutPiece_DetachFromBar>();
            services.AddScoped<IS_ProductionCutPiece_SealToBar, SR_ProductionCutPiece_SealToBar>();
            services.AddScoped<IS_ProductionCutPiece_SetBarOutOfStock, SR_ProductionCutPiece_SetBarOutOfStock>();
            services.AddScoped<IS_ProductionSeries_CompleteCutting, SR_ProductionSeries_CompleteCutting>();
            services.AddScoped<IS_ProductionSeries_Get, SR_ProductionSeries_Get>();
            services.AddScoped<IS_ProductionSeries_SetBarOutOfStockFlag, SR_ProductionSeries_SetBarOutOfStockFlag>();
            services.AddScoped<IS_ProductionSeries_SetSupplyFlag, SR_ProductionSeries_SetSupplyFlag>();
            ...
            // User
            // Services métier de session : consomment un IC_ ou un IQ_ (mutation ou lecture via DbContext partagé) -> Scoped.
            ...
            services.AddScoped<IS_UserApp_Create, SR_UserApp_Create>();
            services.AddScoped<IS_UserApp_Update, SR_UserApp_Update>();
        ...
        private static void RDI_Settings(IServiceCollection services)
        {
            ...
            // Business
            services.AddSingleton<ISE_UseCase, SE_UseCase>();
        ...
        private static void RDI_UseCases(IServiceCollection services)
        {
            ...
            //   (§3.10 du 0230). Non transactionnel par construction, mais consomme cinq
            //   dépendances Scoped qui dictent sa portée (P4-bis, §4.10.10 ; R-4.10.14) :
            //   trois Query Handlers Scoped (IQ_UserApp, IQ_UserAppSession, IQ_AppList) qui
            //   touchent le DbContext partagé, et deux UseCases Scoped consommés en
            //   sous-séquence (IU_UserAppSession_Open, IU_UserAppPageRight_Apply) au titre
            //   de la chaîne UC → UC normalisée R-4.14.21. Consomme par ailleurs le Service
            //   d'infrastructure IS_SingleInstanceGuard Singleton (garde d'unicité d'instance
            //   sur la session Windows courante) en tête de séquence orchestrée, introduit
            //   par le fil d'Extension UC_ApplicationOnStart_Extension — dépendance Singleton
            //   sans incidence sur la portée du UseCase. -> Scoped.
            ...
            // UC_CloseCommand_Check : détection périodique de la nécessité de fermeture de
            //   l'application courante (commande de session active ou non-accessibilité), avec
            //   délégation terminale à IU_CloseApplication mode delay (chaîne UC → UC normalisée
            //   R-4.14.21). Non transactionnel par construction, mais consomme trois dépendances
            //   Scoped (IQ_Generic<UserAppSessionCommand>, IQ_AppList, IU_CloseApplication)
            //   -> Scoped (P4-bis, §4.10.10 ; R-4.10.14).
            services.AddScoped<IU_CloseCommand_Check, UC_CloseCommand_Check>();
            ...
            // Business
            // UC_BarOptimization : ouvre une transaction sur le DbContext partagé et consomme
            //   IQ_VwProductionCutPieceFull + IQ_Generic<vw_CuttingScrapStock_Full> +
            //   IS_ProductionBar_Create / IS_ProductionCutPiece_AssignToBar /
            //   IS_CuttingScrapStock_Reserve (Scoped) ; IS_CuttingOptimizer / IU_LogAndNotify
            //   (Singleton) sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_BarOptimization, UC_BarOptimization>();
            // UC_BarRefusal : ouvre une transaction sur le DbContext partagé et consomme
            //   IQ_Generic<ProductionBar> + IS_ProductionBar_Reject /
            //   IS_ProductionCutPiece_DetachAndRefuse / IS_CuttingScrapStock_Withdraw /
            //   IS_LifecycleAction_Add (Scoped) ; IS_AppContext / IU_LogAndNotify (Singleton)
            //   sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_BarRefusal, UC_BarRefusal>();
            // UC_BarValidation : ouvre une transaction sur le DbContext partagé et consomme
            //   IQ_Generic<ProductionBar> + IQ_VwProductionCutPieceFull +
            //   IS_ProductionBar_Validate / IS_ProductionBar_UpdateOptimization /
            //   IS_ProductionBar_Reject / IS_ProductionCutPiece_SealToBar /
            //   IS_ProductionCutPiece_DetachFromBar / IS_ProductionCutPiece_AssignAndSealToBar /
            //   IS_CuttingScrapStock_Withdraw / IS_ProductionSeries_SetSupplyFlag /
            //   IS_LifecycleAction_Add (Scoped) ; IS_AppContext / IS_CuttingOptimizer /
            //   IU_LogAndNotify (Singleton) sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_BarValidation, UC_BarValidation>();
            // UC_ProductionBar_SetOutOfStock : ouvre une transaction sur le DbContext partagé et
            //   consomme IQ_Generic<ProductionBar> + IS_ProductionBar_SetOutOfStock /
            //   IS_ProductionCutPiece_SetBarOutOfStock / IS_ProductionSeries_SetBarOutOfStockFlag /
            //   IS_LifecycleAction_Add (Scoped) ; IS_AppContext / IU_LogAndNotify (Singleton)
            //   sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_ProductionBar_SetOutOfStock, UC_ProductionBar_SetOutOfStock>();
            // UC_ProductionSeries_CompleteCutting : ouvre une transaction sur le DbContext partagé
            //   et consomme IQ_VwProductionCutPieceFull + IS_ProductionSeries_CompleteCutting /
            //   IS_CuttingScrapStock_ReleaseBySeries / IS_LifecycleAction_Add (Scoped) ;
            //   IS_AppContext / IU_LogAndNotify (Singleton) sans impact sur la portée
            //   (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_ProductionSeries_CompleteCutting, UC_ProductionSeries_CompleteCutting>();

            // User
            // UC_UserApp_Create : ouvre une transaction sur le DbContext partagé et consomme
            //   IQ_UserApp + IS_UserApp_Create (Scoped) ; IS_Hashing / IS_Notification /
            //   IU_LogAndNotify (Singleton) sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_UserApp_Create, UC_UserApp_Create>();
            // UC_UserApp_Update : ouvre une transaction sur le DbContext partagé et consomme
            //   IQ_UserApp + IS_UserApp_Update (Scoped) ; IS_Hashing / IS_Notification /
            //   IU_LogAndNotify (Singleton) sans impact sur la portée (P4-bis, §4.10.10) -> Scoped.
            services.AddScoped<IU_UserApp_Update, UC_UserApp_Update>();
            ...
            // UC_UserAppSession_Close consomme en outre ISE_User + IU_Navigation (Singleton) en sous-séquence
            // post-commit : dépendances Singleton sans impact sur la portée du couple (P4-bis, §4.10.10) -> Scoped inchangé.
            ...
            // UC_UserAuthenticate : consomme IQ_UserApp + IU_UserAppSession_Open / IU_UserAppPageRight_Apply (Scoped)
            //   en sous-séquence -> Scoped.
            services.AddScoped<IU_UserAuthenticate, UC_UserAuthenticate>();
        ...
        private static void RDI_IViewModels(IServiceCollection services)
        {
            services.AddSingleton<IV_Page04>(sp => sp.GetRequiredService<VM_Page04>());
            services.AddSingleton<IV_Page20>(sp => sp.GetRequiredService<VM_Page20>());
        }
        ...
        private static void RDI_ViewModels(IServiceCollection services)
        {
            ...
            services.AddSingleton<VM_MH04>();
            services.AddSingleton<VM_MH10>();
            services.AddSingleton<VM_MH11>();
            services.AddSingleton<VM_MH12>();
            services.AddSingleton<VM_MH13>();
            services.AddSingleton<VM_MH20>();
            services.AddSingleton<VM_MH21>();
            services.AddSingleton<VM_MH22>();
            ...
            // Pages
            services.AddSingleton<VM_Page00>();
            services.AddSingleton<VM_Page01>();
            services.AddSingleton<VM_Page02>();
            services.AddSingleton<VM_Page03>();
            services.AddSingleton<VM_Page04>();
            services.AddSingleton<VM_Page10>();
            services.AddSingleton<VM_Page11>();
            services.AddSingleton<VM_Page12>();
            services.AddSingleton<VM_Page13>();
            services.AddSingleton<VM_Page20>();
            services.AddSingleton<VM_Page21>();
            services.AddSingleton<VM_Page22>();
