...
namespace DG244Cutting.C_Infrastructure.Repositories.DIGIT_TRY
{
    ...
    /// qu'à une table, à l'instar de <c>CR_VwProductionChassisFull</c>. Elle applique le
    ...
    /// </para>
    /// <para>
    /// Objectif : servir au quatrième onglet de la Page11 la composition en barres retenues par
    /// l'optimisation pour une série de production, à raison d'une ligne par barre. Ces barres
    /// proviennent soit du stock de chutes issues de séries antérieures, soit du stock de barres
    /// neuves, et portent cinq indicateurs d'état jalonnant leur parcours. La vue expose
    /// quatre-vingt-deux colonnes ; l'écran en affiche seize. La lecture rapatrie dix-huit champs
    /// - les seize champs d'affichage, plus deux champs de service non affichés dédiés à
    /// l'identification des lignes et à la vérification de cohérence du lot, et concourant aux
    /// critères d'ordonnancement mis à la disposition de l'appelant.
    /// </para>
    /// <para>
    /// Portée du résultat : la lecture ne filtre pas les enregistrements marqués comme
    /// logiquement supprimés, à la différence des autres lectures du projet. Le refus d'une barre
    /// par l'opérateur marque l'enregistrement de cette façon, et l'écran doit afficher ces barres
    /// refusées avec leur motif : elles font partie intégrante du résultat attendu. Une série dont
    ...
    /// dix-huit méthodes du socle imposerait de matérialiser les quatre-vingt-deux colonnes puis
    /// d'en écarter soixante-quatre en mémoire, ce qui ferait perdre la réduction côté base -
    ...
    /// <list type="bullet">
    ///   <item><description>
    ///     Implémenter la lecture projetée déclarée par <see cref="IR_VwProductionBarFull"/>,
    ...
    ///     et ne recourt pas à <c>IgnoreQueryFilters</c> : aucun filtre global n'est configuré sur
    ///     le contexte de données, et la vue ne filtre pas l'indicateur de suppression de sa table
    ///     pilote.
    ///   </description></item>
    ///   <item><description>
    ...
    public class CR_VwProductionBarFull : CR_Generic<vw_ProductionBar_Full>, IR_VwProductionBarFull
    {
        ...
        /// </exception>
        public async Task<List<DTO_VwProductionBarFull_P11>> GetByProductionSeriesIdForP11AsNoTrackingAsync(
            string caller,
            ...
            CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                return await _context.Set<vw_ProductionBar_Full>()
                    ...
                    .Select(v => new DTO_VwProductionBarFull_P11
                    {
