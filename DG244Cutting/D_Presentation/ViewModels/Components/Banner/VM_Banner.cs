...
namespace DG244Cutting.D_Presentation.ViewModels.Components.Banner
{
    ...
    ///   <see cref="HasUnreadMessages"/> et <see cref="LanguageFlagUri"/> bindables
    ///   en lecture, miroir INPC de <see cref="ISE_User.AppUserFullName"/>,
    ///   <see cref="ISE_User.AppUserId"/> &gt; 0, <see cref="ISE_App.HasUnreadMessages"/>
    ///   et <see cref="ISE_Flag.AppFlagUri"/>.</item>
    ///   <item>Exposer <see cref="MessageIconUri"/>, <see cref="MessageUnreadIconUri"/>
    ///   et <see cref="AppCloseIconUri"/> bindables en lecture, immuables, lues
    ///   au constructeur depuis le référentiel statique intra-couche
    ...
    ///   <item>Aucun désabonnement explicite des handlers
    ///   <see cref="INotifyPropertyChanged.PropertyChanged"/> des Settings — le
    ///   ViewModel est Singleton et sa durée de vie est alignée sur celle du
    ///   processus, qui assure naturellement la libération.</item>
    ...
    public class VM_Banner : INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// Obtient l'URI de l'icône du bouton Fermer de la bannière.
        /// </summary>
        /// <remarks>
        /// <para>Contexte : Propriété bindable immuable initialisée au
        /// constructeur à partir de <see cref="RS_Icons.AppCloseBlue_Source"/>.</para>
        /// </remarks>
        public Uri AppCloseIconUri { get; }
        ...
        ///   <see cref="CancellationTokenSource"/> local au ViewModel, §4.6.5).</item>
        ///   <item>Initialisation synchrone des quatre backing fields observables
        ...
        ///   <item>Initialisation des trois propriétés immuables <see cref="Uri"/>
        ...
        ///   bindables observables du ViewModel.</item>
        /// </list>
            ...
            IU_LogAndNotify logAndNotify)
        {
            ...
            // (2) Initialisation des champs locaux _callee et _cts.
            _callee = GetType().Name;
            _cts = new CancellationTokenSource();

            // (3) Initialisation synchrone des quatre backing fields observables
            ...
            // (4) Initialisation des trois propriétés immuables Uri depuis RS_Icons.
            MessageIconUri = RS_Icons.IconEmail_Source;
            MessageUnreadIconUri = RS_Icons.IconEmailNotRead_Source;
            AppCloseIconUri = RS_Icons.AppCloseBlue_Source;
        ...
        // Aucune méthode publique exposée par VM_BA_MainWindow.
        ...
        private async Task NavigateUserAsync()
        {
            ...
            try
            {
                await _uNavigation.NavigateToPageAsync(callChain, "Page90", _cts.Token);
            }
            catch (OperationCanceledException)
            {
                // Annulation amont légitime — silencieux, pas de journalisation.
        ...
        /// (au présent stade, <see cref="ISE_App.HasUnreadMessages"/>).</para>
        /// <para>Objectif : Synchroniser le backing field local
        /// <c>_hasUnreadMessages</c> avec la valeur courante de
        /// <see cref="ISE_App.HasUnreadMessages"/> et déclencher la notification
        /// INPC correspondante via <see cref="SetField{T}"/>.</para>
        /// </remarks>
        ...
        /// <see cref="ISE_User.AppUserId"/> &gt; 0.</para>
        /// </remarks>
        /// <param name="sender">Source de l'événement (l'instance Singleton de
