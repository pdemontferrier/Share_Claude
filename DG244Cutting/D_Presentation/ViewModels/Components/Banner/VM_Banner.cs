...
namespace DG244Cutting.D_Presentation.ViewModels.Components.Banner
{
    ...
    ///   <item>Héberger deux boucles de polling asynchrones encapsulant les
    ///   invocations périodiques de <see cref="IU_UserAppMessage_CheckUnread"/>
    ///   (cadence <see cref="ISE_App.MessageCheckDelay"/>, gating sur
    ///   <see cref="ISE_App.IsConnected"/> et <see cref="IsBannerVisible"/>) et de
    ///   <see cref="IU_DigitTryDb_TestConnection"/> (cadence
    ///   <see cref="ISE_App.DatabaseCheckInterval"/>, sans gating). Chaque boucle
    ...
    /// Le présent ViewModel héberge deux boucles de polling asynchrones
    /// (<see cref="UnreadMessagesPollingLoopAsync"/> et
    /// <see cref="DatabaseConnectivityPollingLoopAsync"/>), amorcées en
    /// fire-and-forget par leurs deux méthodes d'amorçage idempotent dédiées
    /// (<see cref="TryStartUnreadMessagesPollingLoop"/> et
    /// <see cref="TryStartDatabaseConnectivityPollingLoop"/>). Cette posture
    ...
    /// naturel des deux boucles, dont la cadence est portée par les Settings
    /// applicatifs (<see cref="ISE_App.MessageCheckDelay"/>,
    /// <see cref="ISE_App.DatabaseCheckInterval"/>). L'annulation coopérative
    ...
    /// dans les deux méthodes asynchrones de boucle et dans leurs deux amorces
    ...
    public class VM_Banner : INotifyPropertyChanged
    {
        ...
        ///   Initialisation des trois drapeaux internes <c>_unreadLoopStarted</c>,
        ///   <c>_connectivityLoopStarted</c> et <c>_recoveryInProgress</c> à
        ...
        ///   via <see cref="TryStartDatabaseConnectivityPollingLoop"/>
        ...
        ///   <c>_isBannerVisible</c> est déjà vrai à l'instanciation. Les deux
            ...
            IU_LogAndNotify logAndNotify)
        {
            ...
            // (7) Amorçage idempotent des deux boucles de polling — la boucle de
            //     connectivité est amorcée inconditionnellement ; la boucle des
            //     messages non lus n'est amorcée à ce stade que si la bannière est
            //     déjà visible à l'instanciation (cas peu probable mais
            //     fonctionnellement légitime ; les autres cas seront traités par
            //     OnSeUserPropertyChanged à toute notification de IsBannerVisible).
