...
{
    /// <summary>
    /// Service métier chargé de marquer une barre comme sortie du stock.
    /// </summary>
    public class SR_DecoupeBarre_UpdateApproDone : IS_DecoupeBarre_UpdateApproDone
    {
        private readonly string _callee;
        private readonly IQ_DecoupeBarre _qhDecoupeBarre;
        private readonly IC_DecoupeBarre _chDecoupeBarre;

        public SR_DecoupeBarre_UpdateApproDone(
        ...
        public async Task ExecuteAsync(int decoupeBarreId, DTO_AppContext appCtx, string caller)
        {
            ...
            {
                if (decoupeBarreId <= 0)
                    throw new Ex_Business(callChain,"DBU_01", "No_Er_Bu_33");

                var decoupeBarre = await _qhDecoupeBarre.HandleGetByIdAsync(decoupeBarreId);
                if (decoupeBarre == null)
                    throw new Ex_Business(callChain, $"DBU_02 - ID: {decoupeBarreId}", "No_Er_Bu_34");
